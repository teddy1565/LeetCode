    ...
    public:
        ...
        int count(std::vector<int> point) {
            ...
            for (auto item : search_list) {
                if ((data_points_x.find(item[0]) != data_points_x.end()) && (data_points_x[item[0]].find(item[1]) != data_points_x[item[0]].end())) {
                    max_count += (data_points_x[item[0]][item[1]] * data_points_x[point[0]][item[1]] * data_points_x[item[0]][point[1]]);
                    std::cout << item[0] << ", " << item[1] << " | " << data_points_x[item[0]][item[1]] << std::endl;
                    std::cout << point[0] << ", " << item[1] << " | " << data_points_x[point[0]][item[1]] << std::endl;
                    std::cout << item[0] << ", " << point[1] << " | " << data_points_x[item[0]][point[1]] << std::endl;
                } else {
