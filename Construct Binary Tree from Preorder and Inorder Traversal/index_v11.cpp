...
class Solution {
    ...
    private:
        ...
        TreeNode *build_tree_helper(std::unordered_map<int, int> & inorder_map, std::vector<int> & preorder_list, std::vector<int> & inorder_list, int p_left, int p_right, int i_left, int i_right) {
            if (p_left > p_right || i_left > i_right) {
                return nullptr;
            }

            int inorder_index = inorder_map[preorder_list[p_left]];

            if (inorder_index < i_left || inorder_index > i_right) {
                inorder_index = 0;
            }

            TreeNode *current = new TreeNode(preorder_list[p_left]);

            current->left = build_tree_helper(inorder_map, preorder_list, inorder_list, p_left + 1, p_left + inorder_index - i_left, i_left, inorder_index - 1);
            current->right = build_tree_helper(inorder_map, preorder_list, inorder_list, p_left + inorder_index - i_left + 1, p_right, inorder_index + 1, i_right);
    ...
    public:
        TreeNode* buildTree(std::vector<int>& preorder, std::vector<int>& inorder) {
            ...
            // create a hash map store inorder list element index
            ...
            return build_tree_helper(inorder_map, preorder, inorder, 0, preorder.size() - 1, 0, inorder.size() - 1);
        }
};