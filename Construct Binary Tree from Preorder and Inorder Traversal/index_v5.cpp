...
class Solution {
    ...
    private:
        ...
        TreeNode* build_tree_helper(const int preorder_size, const int inorder_size, std::unordered_map<int, int> & inorder_map, std::vector<int> & preorder, int *index, int parent_index) {
            ...
            int parent_index_for_parent_root = parent_index;
            ...
            if (((*index) + 1) < preorder_size) {
                int left_index = preorder[(*index) + 1];
                int inorder_index_for_left = inorder_map[left_index];

                if (parent_index_for_parent_root >= 0 && inorder_index_for_left > parent_index_for_parent_root) {
                    std::cout << (*index) << " | " << inorder_index_for_left << " | " << parent_index_for_parent_root << " | " << inorder_index_for_root << " | " << left_index << std::endl;
                    ...
                    } else if (inorder_index_for_left > inorder_index_for_root) {
                        
                        std::cout << "inorder_index_for_left > inorder_index_for_root" << std::endl;
                        // (*index) += 1;
                        // root->right = build_tree_helper(preorder_size, inorder_size, inorder_map, preorder, index, inorder_index_for_root);
                    }
                    return root;
            ...
            if (root->right == nullptr && ((*index) + 1) < preorder_size) {
                (*index) += 1;
                root->right = build_tree_helper(preorder_size, inorder_size, inorder_map, preorder, index, parent_index_for_parent_root);
            }
