...
int** threeSum(int* nums, int numsSize, int* returnSize, int** returnColumnSizes) {
    *returnSize = 0;
    ...
    for (int i = 0; i < (50 * mem_scale_x); i++) {
    ...
    for (int i = 0; i < numsSize - 2; i++) {
        for (int left = i + 1, right = numsSize - 1; (left <= numsSize - 2) && (right >= i + 2) && left != (right + 1) && left != right;) {
            ...
            if (sum == 0) {
                ...
                if((*returnSize) == (50 * mem_scale_x)) {
                    int **temp_p = realloc(p, sizeof(int **) * 50 * (mem_scale_x + 1));
                    if (temp_p != NULL) {
                        p = temp_p;
                        temp_p = NULL;
                        for (int p_i = (50 * mem_scale_x); p_i < (50 * (mem_scale_x + 1)); p_i++) {
