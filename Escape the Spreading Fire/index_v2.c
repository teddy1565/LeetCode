...
int maximumMinutes(int** grid, int gridSize, int* gridColSize) {
    ...
    int fire_timer = 0;
    int user_timer = 0;
    int fire_step_timer = 0;
    int fire_timer_stop_signal = 0;
    int esc_success_signal = 0;
    int user_timer_stop_signal = 0;
    int user_timer_with_fire = 0;
    int lag_user_queue_index = 0;
    int lag_fire_queue_index = 0;
    ...
    while (user_queue_index - offset_user_queue_index > 0) {
        ...
        if (fire_queue_index != update_fire_queue_index) {
            lag_fire_queue_index = fire_queue_index;
        }
        ...
        if (fire_timer_stop_signal == 0) {
            user_timer_with_fire++;
        }
        fire_step_timer++;
        if (user_queue_index != update_user_queue_index) {
            lag_user_queue_index = user_queue_index;
        }
    ...
    printf("user_timer: %d\n", user_timer);
    printf("user_timer_sig: %d\n", user_timer_stop_signal);
    printf("user_timer_with_fire: %d\n", user_timer_with_fire);
    printf("fire_step_timer: %d\n", fire_step_timer);
    printf("user_queue_index: %d\n", user_queue_index);
    printf("fire_queue_index: %d\n", fire_queue_index);
    printf("offset_user_queue_index: %d\n", offset_user_queue_index);
    printf("lag_user_queue_index: %d\n", lag_user_queue_index);
    printf("lag_fire_queue_index: %d\n", lag_fire_queue_index);

    // if (esc_success_signal == 1 && fire_timer_stop_signal == 1 && user_timer_stop_signal == 1 ) {

    // } else 
    if (esc_success_signal == 0 && fire_timer == 0 && fire_timer_stop_signal == 1 && user_timer == 1) {
        return ESC_MIN;
    } else if (esc_success_signal == 0 && fire_timer == 0 && fire_timer_stop_signal == 1) {
        return ESC_MAX;
    } else if (esc_success_signal == 0 && fire_timer == 1) {
        return ESC_MIN;
