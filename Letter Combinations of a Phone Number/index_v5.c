    ...
    *returnSize = total;
    ...
    for (int i = 0; i < 1; i++) {
        for (int j = 0; j < total; j++) {
            ...
            answer[j][i] = key_board[key_board_row][j / rows];
        }
    }

    int divisor = rows / cols; 
    for (int i = 1; i < digits_size; i++) {
        for (int j = 0; j < total; j++) {
            int key_board_row = digits[i] - CHR_OFFSET;
            // printf("%d %d %d\n", j, i, key_board_row);
            answer[j][i] = key_board[key_board_row][(j / divisor) % digits_length_map[key_board_row]];
        }
        if (i < digits_size - 1) {
            divisor = divisor / digits_length_map[digits[i] - CHR_OFFSET];
        }
    }
