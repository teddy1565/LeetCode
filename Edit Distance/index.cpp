...
class Solution {
    public:
        int minDistance(std::string word1, std::string word2) {
            ...
            // initial, when word1[i] transfer to word2[0], need how many step
            for (int i = 0; i <= word1_size; i++) {
                dp[i][0] = i;
            }

            // initial, when word2[i] transfer to word1[0], need how many step
            ...
            for (int i = 1; i <= word1_size; i++) {
                for (int j = 1; j <= word2_size; j++) {
                    ...
                    if (word1[i - 1] == word2[j - 1]) {
                        // if word1[i - 1] == word[j - 1], just repeat and transfer prev status
                        dp[i][j] = dp[i - 1][j - 1];
                    } else {
                        // else, it step is minimum last status step + 1
