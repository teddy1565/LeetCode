...
void grid_bfs(char** grid, int row_size, int col_size, int p_index_x, int p_index_y, int (*island_list)[2], int *island_index) {
    ...
    while (update_temp_list_index != temp_index) {
        for (int i = update_temp_list_index; i < temp_index; i++) {
            for (int j = 0; j < 4; j++) {
                ...
                int find_repeat = 0;
                for (int k = 0; k < island_temp_list_index; k++) {
                    if (island_temp_list[k][0] == x && island_temp_list[k][1] == y) {
                        find_repeat = 1;
                        break;
                    }
                }
    ...
    for (int i = 0; i < island_temp_list_index; i++) {
        (island_list)[(*island_index)][0] = island_temp_list[i][0];
        (island_list)[(*island_index)][1] = island_temp_list[i][1];
        (*island_index) += 1;
    }
}
...
int numIslands(char** grid, int gridSize, int* gridColSize) {
    ...
    int island_postive[TABLE_SIZE][2] = {0};
    for (int i = 0; i < row_size; i++) {
        for (int j = 0; j < col_size; j++) {
            if (grid[i][j] == '1') {
                int find_repeat = 0;
                for (int k = 0; k < island_index; k++) {
                    if (island_postive[k][0] == i && island_postive[k][1] == j) {
                        find_repeat = 1;
                        break;
                    }
                }
                ...
                grid_bfs(grid, row_size, col_size, i, j, island_postive, &island_index);
                island_count += 1;
            }
        }
    }
